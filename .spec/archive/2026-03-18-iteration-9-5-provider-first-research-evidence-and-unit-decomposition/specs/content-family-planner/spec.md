...
**Reason**: `family-leaf-doc` 的职责已经被通用 `knowledge-unit-decomposition` 能力收编，叶子页不再以 family 专用页面类型表达。
**Migration**: 现有 family 叶子页需求必须改为基于 `DecompositionProfile` 规划的通用 leaf KnowledgeUnit，并通过对应 domain/index/page path 落盘。

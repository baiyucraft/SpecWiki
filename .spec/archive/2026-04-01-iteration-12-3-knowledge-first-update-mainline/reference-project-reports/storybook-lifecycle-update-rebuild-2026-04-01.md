...
- 本报告不覆盖 UniSpec 意义上的 `cold restore / warm restore preflight / cache rebuild`
...
但这里的含义要收紧：它只证明“源码回退后可以成功进入 rebuild，并回到 fresh”，不证明 UniSpec 语义下的 `restore baseline`、`.wiki/.cache/**` 缺失恢复，或“只做 cache rebuild 且没有隐式重跑 planning/research/compose/assemble”。
...
- 任务 wording 已明确排除了 `dagger` 与 UniSpec 语义下的 `cold restore / warm restore preflight / cache rebuild`

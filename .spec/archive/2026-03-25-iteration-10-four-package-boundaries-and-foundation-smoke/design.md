...
`.docs/roadmap/implementation-roadmap.md` 已把新的 `迭代 10` 定义为“四包拆分与基础测试收口”，但当前工作区仍只有单一 [`crates/wiki-core`](E:/project/!byAI/spec-wiki/crates/wiki-core) 成员，且 [`lib.rs`](E:/project/!byAI/spec-wiki/crates/wiki-core/src/lib.rs) 继续把 `domain / generation / llm / repo / storage / transport / workflows` 一并导出。当前主问题已经不是“有没有 2.0 主链”，而是这条主链是否被错误的工程边界持续反向定义。
...
- 把工作区正式拆成 `wiki-model / wiki-index / wiki-knowledge / wiki-runtime` 四个 crate，并让依赖方向与 `.wiki/06-设计文档/00-总体设计.md` 一致。
...
- 让拆层后的基础 workflow 不回退，并满足本轮收紧后的测试约束：crate/workspace 测试、生命周期脚本、`storybook + dagger` 守门样本、`.wiki/02-开发指南/00-代码注释规范.md` 单独检查。

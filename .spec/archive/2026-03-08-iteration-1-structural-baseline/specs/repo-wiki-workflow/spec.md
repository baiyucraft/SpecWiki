...
系统 MUST 在有效本地代码目录上执行完整初始化，生成第一阶段所需的 Wiki 页面、metadata 和 cache，并在页面规划阶段基于递归模块树生成层级化页面。Git 信息在存在时可作为元数据补充，但不得成为初始化前提。
...
系统 MUST 基于正式 Wiki 页面、metadata 与模块结构返回结构化查询结果，而不是仅输出非结构化文本。`query` 必须优先命中页面、模块、源码和关系等结构化索引，Markdown 文本匹配仅可作为回退路径。

#### Scenario: 查询命中结构化对象
- **WHEN** 用户提供查询词且存在相关 Wiki 页面或模块结构

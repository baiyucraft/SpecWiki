...
`.wiki/06-设计文档/00-总体设计.md`、`.wiki/06-设计文档/01-Runtime设计.md` 与 `.docs/roadmap/implementation-roadmap.md` 已经把迭代 11 的边界写得很清楚：这一轮要强化 `wiki-index`，让“方法定位、入口定位、基础影响分析”优先走 facts/index，而不是继续把查询语义堆在 runtime 或 page 上。当前真实代码与真实产物都说明这件事还没有完成。
...
- `.docs/roadmap/implementation-roadmap.md` 已明确迭代 11 的重点是 `wiki-index` 初步可用，而不是 transport/Agent 协议定版。

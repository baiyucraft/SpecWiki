---
title: 总体设计
description: Repo Wiki Core + Agents 的总体定位、四包架构和核心原则
owner: architecture
updated: 2026-07-17
---
...
当前项目定位为 `Repo Wiki Core + Agents` 体系。core 的内部架构已经收束为四个 crate，宿主接入、bootstrap 与全局 CLI 形态则单独由 `Agents` 层承载，具体见 [02-Agents设计](./02-Agents设计.md)：
...
- `wiki-runtime` 负责 Runtime workflow、查询路由、投影和 `.wiki/` 生命周期
- `packages/spec-wiki` 与 `Agents` 负责宿主 bootstrap、宿主资产编排和 Runtime forwarding，不承载 Wiki 业务规则；宿主接入设计见 [02-Agents设计](./02-Agents设计.md)

## 设计文档索引

| 文档 | 内容 | 状态 |
|------|------|------|
| [01-Runtime设计](./01-Runtime设计.md) | 整个 wiki 系统的总运行架构、`.wiki/` 结构、query route、生命周期与恢复策略 | **当前** |
| [02-Agents设计](./02-Agents设计.md) | `spec-wiki` 的宿主接入、bootstrap、runtime forwarding 与宿主扩展模型 | **当前** |
| [implementation-roadmap](../../.docs/roadmap/implementation-roadmap.md) | 3.0 的拆包顺序、迁移路线、验收重点、**阶段性迭代参考** | 阶段材料 |
| [03-核心场景](./03-核心场景.md) | 第一版 9 个核心用户故事 | **当前** |
| [04-扩展场景](./04-扩展场景.md) | 第一版之外的重要扩展场景 | **当前** |
| [05-产品基线与设计治理](./05-产品基线与设计治理.md) | 3.0 产品范围、版本 authority、状态证据和设计完成规则；本页继续负责架构内容 | **当前治理基线** |
...
### 7. 宿主接入采用 Codex-first

Codex 是当前唯一 `reference host` 和首要兼容目标；Claude、CodeBuddy 是 `compatible host`。公共宿主合同先在 shared kernel 中定义，再投影到 Codex repo-local skills，最后验证 Claude 与 CodeBuddy 的兼容投影。

CodeBuddy hooks/settings 是可选的宿主专属机制，不是公共架构前提。机制能力更丰富不代表它可以反向定义 trigger taxonomy、Runtime query 语义、bridge transport 或 provider session 生命周期。稳定触发合同见 [host-trigger-contract capability](../05-规格基线/capabilities/host-trigger-contract/spec.md)。
...
- `wiki-runtime`：Runtime workflow、查询、投影与生命周期执行面

更细的包边界、运行协作和主链说明，见 [01-Runtime设计](./01-Runtime设计.md)。宿主接入、bootstrap 与扩展适配设计见 [02-Agents设计](./02-Agents设计.md)。
...
具体运行顺序、query route 和生命周期细节，见 [01-Runtime设计](./01-Runtime设计.md)。

## 与用户场景的关系

3.0 的验收不再只看“能不能生成 `.wiki/*.md`”，而要对应 [03-核心场景](./03-核心场景.md) 和 [04-扩展场景](./04-扩展场景.md) 的真实使用场景。

核心对应关系：

- 项目理解、改动入口定位、影响分析：优先依赖 `wiki-index`
- 规则读取、知识更新、避坑沉淀、冲突治理、主动声明规范：优先依赖 `wiki-knowledge`
- 本地恢复、runtime 生命周期与查询路由：优先依赖 `wiki-runtime`
- 宿主 bootstrap、trigger 投影与 runtime forwarding：优先依赖 `packages/spec-wiki` / `Agents`
...
- 项目集跑批与生命周期验证的具体执行方式，见 [implementation-roadmap](../../.docs/roadmap/implementation-roadmap.md)
- 日常测试约束以 [AGENTS.md](../../AGENTS.md) 中的开工检查和文档边界章节为准

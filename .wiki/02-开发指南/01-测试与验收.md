...
- `formal gate`：由 schema、artifact、restore、query/status 等正式合同拥有 blocker decision。
- `primary gate`：对本 change 声明的 primary fixtures 和 required gates 给出收口结论，不替代 formal gate。
- `baseline guard`：批量项目集与生命周期回归，只引用已归属 failure，不重复制造 blocker owner。
- `diagnostic`：结构化观察与趋势信息，不拥有 formal blocker，也不能把 required coverage 缺失伪装成 pass。

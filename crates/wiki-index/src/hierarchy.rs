...
pub fn build_module_tree_with_graph(
    ...
    build_module_tree_with_graph_and_assist(report, graph_summary, None)
}

/// 基于扫描结果、graph summary 和可选 LLM runtime 构建模块树。
pub fn build_module_tree_with_graph_and_assist(
    report: &ScanReport,
    graph_summary: &GraphSummary,
    ...
    let (modules, cross_module_edges) = match llm_runtime {
        Some(assist) => {
            let discovery = discover_module_roots(report, Some(&mut *assist));
            ...
            let cross_module_edges =
                build_cross_module_edges(report, &modules, graph_summary, Some(&mut *assist));
            (modules, cross_module_edges)
        }
        None => {
            let discovery = discover_module_roots(report, None);
            let root_module = ModuleNode {
                id: root_id.clone(),
                name: root_name,
                kind: "repository".to_string(),

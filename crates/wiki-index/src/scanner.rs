...
pub fn scan_repo_with_boundary(
    ...
    scan_repo_with_boundary_and_assist(root, extra_ignore_paths, extra_include_paths, None)
}

/// 带可选 LLM Uncertainty Gate 的扫描入口。
pub fn scan_repo_with_boundary_and_assist(
    root: &Path,
    extra_ignore_paths: &[String],
    extra_include_paths: &[String],
    ...
    match llm_runtime {
        Some(assist) => {
            apply_assist_file_purpose_overrides(&mut files, pending_file_purposes, Some(assist))?;
        }
        None => {
            apply_assist_file_purpose_overrides(&mut files, pending_file_purposes, None)?;
        }
    }
...
fn apply_assist_file_purpose_overrides(

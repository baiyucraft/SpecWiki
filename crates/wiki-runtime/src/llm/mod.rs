...
use wiki_index::assist::{self as index_assist, FactsAssist};
...
impl FactsAssist for LlmRuntime<'_, '_> {
    fn classify_file_purposes(
        &mut self,
        inputs: &[index_assist::FilePurposeAssistInput],
    ) -> io::Result<Vec<Option<FilePurpose>>> {
        let converted = inputs
            .iter()
            .map(|input| FilePurposeAssistInput {
                path: input.path.clone(),
                kind: input.kind.clone(),
                language: input.language.clone(),
                file_size: input.file_size,
                deterministic: input.deterministic.clone(),
                preview: input.preview.clone(),
            })
            .collect::<Vec<_>>();
        LlmRuntime::classify_file_purposes(self, &converted)
    }

    fn decide_top_level_promotions(
        &mut self,
        inputs: &[index_assist::TopLevelPromotionAssistInput],
    ) -> io::Result<Vec<Option<bool>>> {
        let converted = inputs
            .iter()
            .map(|input| TopLevelPromotionAssistInput {
                root_path: input.root_path.clone(),
                score: input.score,
                ...
                has_subdirs: input.has_subdirs,
                languages: input.languages.clone(),
                tags: input.tags.clone(),
            })
            .collect::<Vec<_>>();
        LlmRuntime::decide_top_level_promotions(self, &converted)
    }

    fn classify_module_kind(
        &mut self,
        input: &index_assist::ModuleKindAssistInput,
    ) -> io::Result<Option<String>> {
        let converted = ModuleKindAssistInput {
            root_path: input.root_path.clone(),
            tags: input.tags.clone(),
            explicit_root: input.explicit_root,
            has_children: input.has_children,
            has_main_entry: input.has_main_entry,
            has_app_source: input.has_app_source,
            has_infra_files: input.has_infra_files,
            workspace_member: input.workspace_member,
        };
        LlmRuntime::classify_module_kind(self, &converted)
    }

    fn keep_dependency_edges(
        &mut self,
        inputs: &[index_assist::DependencyAssistInput],
    ) -> io::Result<Vec<Option<bool>>> {
        let converted = inputs
            .iter()
            .map(|input| DependencyAssistInput {
            ...
            .collect::<Vec<_>>();
        LlmRuntime::keep_dependency_edges(self, &converted)
    }
}
#[cfg(test)]
mod tests {

...
fn collect_symbol_matches(
    ...
    let hits = match index_store.search_symbols(needle, 24) {
        Ok(hits) => hits,
        Err(_) => return,
    };
...
fn symbol_match_reasons(hit: &wiki_index::store::SymbolSearchHit, needle: &str) -> Vec<&'static str> {
    let mut reasons = Vec::new();
...
fn collect_graph_matches(
    ...
    let index_store = SqliteIndexStore::new(repo_root);
    let symbols = match index_store.list_symbols() {
        Ok(symbols) => symbols,
        Err(_) => return,
    };
    if symbols.is_empty() {
    ...
    let traced_call_edges = index_store
        .trace_call_edges_from_seeds(&graph_seed_symbol_ids, 3, 48)
        .unwrap_or_default();
...
fn collect_graph_edge_matches(
    ...
    matched_symbol_ids: &BTreeSet<String>,
    traced_call_edges: &[wiki_index::store::CallTraceHit],
    page_index: &BTreeMap<String, &WikiPageState>,
    module_index: &BTreeMap<String, &ModuleNode>,
    source_path_index: &BTreeMap<String, &SourceState>,
    page_matches: &mut BTreeMap<String, PageMatchState>,
    ...
    let index_store = SqliteIndexStore::new(repo_root);
    let edges = match index_store.list_edges() {
        Ok(edges) => edges,
        Err(_) => return,
    };
...
fn collect_process_graph_matches(
    ...
    let index_store = SqliteIndexStore::new(repo_root);
    let processes = match index_store.list_processes() {
        Ok(processes) => processes,
        Err(_) => return,
    };

    for process in processes {
        let steps = index_store
            .list_process_steps(&process.process_id)
            .unwrap_or_default();
...
fn collect_community_graph_matches(
    ...
    let index_store = SqliteIndexStore::new(repo_root);
    let communities = match index_store.list_communities() {
        Ok(communities) => communities,
        Err(_) => return,
    };

    for community in communities {
        let member_ids = index_store
            .list_community_members(&community.community_id)
            .unwrap_or_default()
            .into_iter()

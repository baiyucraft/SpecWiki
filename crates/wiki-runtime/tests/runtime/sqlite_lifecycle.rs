...
use wiki_runtime::workflows::rebuild::run_rebuild;
use wiki_runtime::workflows::sync::run_sync;
use wiki_runtime::workflows::update::run_update;
...
fn init_creates_db_and_state() {
    ...
    // wiki-state 应该可读
    let state = wiki_runtime::storage::state_store::read_state(repo.path()).unwrap();
    assert!(!state.pages.is_empty());

    // scan cache 和 module tree cache 应该可读
    let scan = wiki_runtime::storage::cache_store::read_scan_cache(repo.path());
    assert!(scan.is_ok());
    let tree = wiki_runtime::storage::cache_store::read_module_tree_cache(repo.path());
    assert!(tree.is_ok());
}
...
fn rebuild_recreates_db() {
    ...
    let state_before = wiki_runtime::storage::state_store::read_state(repo.path()).unwrap();
    ...
    let state_after = wiki_runtime::storage::state_store::read_state(repo.path()).unwrap();
...
fn full_lifecycle_init_update_sync_rebuild() {
    ...
    // 最终状态应该完整
    let final_state = wiki_runtime::storage::state_store::read_state(repo.path()).unwrap();

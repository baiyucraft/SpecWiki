...
#[test]
fn hierarchy_planning_builds_mixed_repo_edges_from_api_and_infrastructure_hints() {
    let fixture = [
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures/mixed-local-repo"),
        PathBuf::from("crates/wiki-core/tests/fixtures/mixed-local-repo"),
        PathBuf::from("tests/fixtures/mixed-local-repo"),
    ...
    let edge_names = module_tree
        ...
        .iter()
        .map(|edge| {
            let source = module_tree
                .module_by_id(&edge.source)
                .map(|module| module.name.clone())
                .unwrap_or_default();
            let target = module_tree
                .module_by_id(&edge.target)
                .map(|module| module.name.clone())
                .unwrap_or_default();

            (source, target, edge.relation_type.clone())
        })
        .collect::<Vec<_>>();
...
#[test]
fn hierarchy_planning_builds_recursive_module_tree_and_nested_paths() {
    let fixture = [
    ...
    let packages = module_tree
        .modules
        .iter()
        .find(|module| module.root_paths.iter().any(|root| root == "packages"))
        .expect("packages group should exist");
    ...
    assert_eq!(auth.parent_id.as_deref(), Some(domain.id.as_str()));
    assert!(pages
        .iter()

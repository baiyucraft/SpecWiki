...
pub fn write_scan_cache(repo_root: &Path, report: &ScanReport) -> io::Result<()> {
    ...
    let json = serde_json::to_string(report).map_err(|e| io::Error::other(e.to_string()))?;
...
pub fn read_scan_cache(repo_root: &Path) -> io::Result<ScanReport> {
    ...
    match sqlite_store::scan_cache_get(&conn, "repo-scan")? {
        Some(json) => serde_json::from_str(&json).map_err(|e| io::Error::other(e.to_string())),
        None => Err(io::Error::new(
            io::ErrorKind::NotFound,
            "repo-scan not found in DB",
        )),
    }
}
...
pub fn write_module_tree_cache(repo_root: &Path, module_tree: &ModuleTree) -> io::Result<()> {
    let conn = sqlite_store::open_db(repo_root)?;
    let json = serde_json::to_string(module_tree).map_err(|e| io::Error::other(e.to_string()))?;
...
pub fn missing_incremental_cache_components(repo_root: &Path, state: &WikiState) -> Vec<String> {
    ...
    if sqlite_store::scan_cache_get(&conn, "repo-scan")
        .ok()
        .flatten()
        .is_none()
    {
    ...
    if sqlite_store::scan_cache_get(&conn, "module-tree")
        .ok()
        .flatten()
        .is_none()
    {
    ...
    if sqlite_store::load_state_rows(&conn).is_err() {

...
- 这意味着现在的增量行为更接近“全树 knowledge/research/compose + 局部页面写回”，而不是 `DESIGN-RUNTIME.md` 里定义的 `detect affected facts -> locate affected knowledge scope -> refresh derived knowledge -> refresh impacted projections`
...
- `DESIGN-RUNTIME.md` 已把“locate affected knowledge scope”写成正式主链；缺的只是具体 contract
...
专项门禁仍优先 `storybook + dagger`。完整项目集 `init` 分析与 `COMMENTING.md` 合规检查继续保留为 tasks/verification 约束，而不是与主专项门禁同权的设计目标。
